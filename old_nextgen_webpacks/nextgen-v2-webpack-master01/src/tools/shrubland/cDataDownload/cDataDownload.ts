  ...
  createExtendedTool(owningBlock) {
    ...
    const extendedTool = {
      ...
      createDefaultVars(): void {
        if (options.fixedDownloadBox) {
          this.hyp =
            (Math.acos(Math.cos(options.maxDownloadBoxWidth / options.earthRadius) * Math.cos(options.maxDownloadBoxHeight / options.earthRadius)) *
              options.earthRadius) /
            2; // spherical law of cosines
          this.a1 = Math.atan(options.maxDownloadBoxWidth / options.maxDownloadBoxHeight);
          this.a2 = Math.atan(options.maxDownloadBoxHeight / options.maxDownloadBoxWidth);
        }
      ...
      extentIsEmpty(extent = this.vector.getSource().getExtent()): boolean {
      ...
      extentIsValid(extent = this.vector.getSource().getExtent()): boolean {
        ...
        if (options.hasOwnProperty('maxArea') && area > options.maxArea) {
          return false;
        }
      ...
      enableForm(): void {
        ...
        if (this.extentIsValid(extent)) {
          if (options.redirectText && !options.alwaysDisplayRedirect) {
       ...
       */
      removeVector(vector = this.vector): void {
        const map = this.getCurrentMap();
        map.removeLayer(vector);
      },
      addVector(vector = this.vector): void {
      ...
      createTranslate(): void {
        ...
        this.mapInteraction.on(
          'translating',
          function () {
            const source = extendedTool.vector.getSource();
      ...
      createDrawBox(): void {
        ...
        layers.forEach((layer) => {
          if (typeof layer.values_.id !== 'undefined' && layer.values_.id === 'dataDownload') this.vector = layer;
        });
      ...
      hidePanelItems(extent): void {
        ...
        if (redirectDownload && downloadPanel && categoryText) {
          if (this.extentIsValid(extent)) {
            ...
            if (!options.alwaysDisplayDownloadPanel) {
              downloadPanel.hide();
            } else {
              downloadPanel.hide();
      ...
      setFeature(tbname?: string): void {
        const lssource = this.vector.getSource();
        ...
        if (!lsfeatur[0]) {
          if (options.fixedDownloadBox) {
            const points = this.getWrappedBoxCoordinates(tbname);

            // getting bounding extent keeps the polygon rectangular instead of trapezoidal
            const bext = boundingExtent([points[0], points[1], points[2], points[3]]);
            this.feature = new Feature({
              geometry: fromExtent(bext),
            });
          } else {
      ...
      openAndEnable(): void {
        if (!options.redirectButton) {
          Ext.getCmp('redirectDownloadBtn').hide();
        }
        if (!options.redirectDownload || !options.alwaysDisplayRedirect) {
          Ext.getCmp('redirectDownload').hide();
        }

        this.createDefaultVars();
        if (options.hasOwnProperty('fixedDownloadBox') && options.fixedDownloadBox) {
          this.setFeature();
        } else {
          this.createDrawBox();
        }
      ...
      parseXMLforCategories(request: XMLHttpRequest): void {
        ...
        const elementTitle = options.hasOwnProperty('workspace') ? options.workspace : layers[0].name.split(':')[0];
        ...
        for (let i = 0; i < categoryTags.length; i += 1) {
          const mcat = categoryTags[i].childNodes[0].nodeValue.replace(' ', '_').toLowerCase();
          ...
          for (let j = 0; j < layers.length; j++) {
            if (clippedRasterName === layers[j].name) {
          ...
          const dat: [string, string, string, string, string, string, boolean] = [
            sourceMetadataPath,
            clippedRasterName,
            sourceFolderName,
            sourceFileName,
        ...
        if (options.hasOwnProperty('yearSelection') && options.yearSelection) {
          for (const [catKey, years] of Object.entries(categoryYears)) {
            if (years.length > 1 && years.includes('2016')) {
              ...
              if (!Array.isArray(pnode.children) || !pnode.children.length) {
                const displayCategory = catKey.replace('_', ' ').replace(/\b\w/g, function (txt) {
                  return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
                });
                const capCatKey = displayCategory.replace(' ', '_');
                const c0BoxLabel = `2016 ${displayCategory} ONLY`;
                const c1BoxLabel = `All ${displayCategory} Years`;
                const c0Name = `${catKey}2016`;
                const c1Name = `${catKey}All`;

                let c0IV = null;
                // for when we want to include files that do not have 2016 in the name for the 2016 only selection
                const keys = Object.keys(options?.categories);
                for (const key of keys) {
                  if (catKey === key) {
                    c0IV = options.categories[key];
                  }
                }
                if (keys && c0IV === null) {
                  c0IV = `${options.categories['default']}${capCatKey}`;
                }
      ...
      updateBBoxCategories(): void {
        ...
        if (this.extentIsValid(extent)) {
          const layers = this.getLayers();
          const geoserver = options.geoserver ? options.geoserver : layers[0].source.wfs;
          const shapeFile = options.shapeFile ? options.shapeFile : layers[0].name;
          const categoryCheckboxes = Ext.getCmp('cbCategories');
          const categoryText = Ext.getCmp('categoryText');
          let url = null;
      ...
      items: [
        {
          ...
          items: [
            ...
            {
              xtype: 'button',
              id: 'redirectDownloadBtn',
              text: 'Download Large Datasets',
        ...
        },
        {
          ...
          width: '100%',
          hidden: !options.alwaysDisplayDownloadPanel,
          items: [
            {
              xtype: 'tbtext',
              ...
              items: [
                {
                  ...
                  listeners: {
                    click(button): void {
                      const lssource = extendedTool.vector.getSource();
                      if (options.hasOwnProperty('fixedDownloadBox') && options.fixedDownloadBox) {
                        const btntxt = button.getText();
                        if (btntxt === 'Clear') {
                          lssource.clear();
                        ...
                        const layers = map.getLayers().array_;
                        layers.forEach((layer) => {
                          if (typeof layer.values_.id !== 'undefined' && layer.values_.id === 'dataDownload') extendedTool.vector = layer;
                        });
                        const categoryCheckboxes = Ext.getCmp('cbCategories');
                    ...
                    enable(button): void {
                      const extent = [this.extendedTool.lon.min, this.extendedTool.lat.min, this.extendedTool.lon.max, this.extendedTool.lat.max];
                      if (this.extendedTool.extentIsEmpty(extent)) {
                        button.setText('Place');
                      } else {
                        button.setText('Clear');
                      }
                ...
                },
                {
                  ...
                  listeners: {
                    click(): void {
                      ...
                      const extent = this.extendedTool.vector.getSource().getExtent();
                      ...
                      if (jsonstr && jsonstr !== '[]' && jsonstr !== '{}' && catstr && this.extendedTool.emailIsValid(this.extendedTool.email)) {
                        ...
                        const url = options.addQueueLocation;
      ...
      listeners: {
        ...
        afterrender(): void {
          ...
          if (urlParams.has('downloadBbox')) {
            ...
            // create middle coordinate from download bbox
            const midLat = (parseFloat(coordArray[1]) + parseFloat(coordArray[0])) / 2;
            const midLon = (parseFloat(coordArray[3]) + parseFloat(coordArray[2])) / 2;
            const middlePoint = [midLon, midLat];
            // const view = this.extendedTool.getCurrentMap().getView();

            // create middle coordinate from download bbox
            const setView = () => {
              this.extendedTool.getCurrentMap().getView().fit(this.extendedTool.feature.getGeometry().getExtent());
              // view.fit(this.extendedTool.bboxCoordinates);
              // console.log(view);
            };
